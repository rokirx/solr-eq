 ...
 */
public abstract class EqDisjunctionScorer extends Scorer {
    ...
    public EqDisjunctionScorer(Weight weight, Scorer[] subScorers, ScoreMode scoreMode) {
        super(weight);
        int scorerIndex = 1;

        // we are sure to have at least one non-null subScorer here
        this.subScorers = new DisiPriorityQueue(countNonNull(subScorers));
        for (Scorer scorer : subScorers) {
            if (null != scorer)
                this.subScorers.add(new EqDisiWrapper(scorer, scorerIndex));
            scorerIndex++;
        }
        ...
        // Compute matchCost as the average over the matchCost of the
        // subScorers.
        ...
        if (hasApproximation == false) { // no sub scorer supports
                                         // approximations
            twoPhase = null;
        } else {
    ...
    private static final int countNonNull(Object[] arr) {
        int sum = 0;
    ...
    private class TwoPhase extends TwoPhaseIterator {
        ...
        // priority queue of approximations on the current doc that have not
        // been verified yet
        ...
        private TwoPhase(DocIdSetIterator approximation, float matchCost) {
            ...
            unverifiedMatches = new PriorityQueue<DisiWrapper>(EqDisjunctionScorer.this.subScorers.size()) {
                @Override
    ...
    public final float score() throws IOException {
        if (curDoc == subScorers.top().doc)
            return curScore;
        curScore = score(getSubMatches());
        curDoc = subScorers.top().doc;
        return curScore;
    }
